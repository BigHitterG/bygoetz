...
export function CommunityGardenApp() {
  ...
  const inventoryHighlighted =
    onboardingStep === "plant" || onboardingStep === "personal-inventory";
  const inventoryIconClass =
    ui.selectedTool === "path"
      ? "cg-path-icon"
      : ui.selectedElementType && ui.selectedTool === ui.selectedElementType
        ? `cg-item-glyph ${getMyGardenElementGlyphClass(ui.selectedElementType)}`
  ...
  const showMyGardenGrowthNudge =
    ...
    (myGarden.preview?.plantingsUsed ?? 0) <
      (myGarden.preview?.plantingLimit ?? GUEST_PLANTING_LIMIT) &&
  ...
  const loadMembership = useCallback(async (activeSession: Session) => {
    ...
    try {
      ...
      setGardenHouse(account.active ? account.house : null);
  ...
  const inspectGardenHouseDisplay = useCallback(
    (key: GardenHouseDisplayKey) => {
      ...
      setGardenHouse((current) => {
        if (!current) return current;
        const displays = current.displays.map((item) =>
          item.key === key ? { ...item, inspectedAt } : item,
        );
        return {
          ...current,
          displays,
      ...
      if (!session) return;
      void fetch("/api/community-garden/accolades", {
        method: "POST",
        headers: {
          authorization: `Bearer ${session.access_token}`,
          "content-type": "application/json",
    ...
    <main className={`cg-root is-${world}-world`}>
      <section className="cg-game-frame" aria-label="Basil garden game">
        ...
        <GardenInventory
          ...
          toggleLocked={inventoryToggleLocked}
          showToggle={false}
          guidePlantChoice={
            onboardingStep === "select-seed" ||
            onboardingStep === "personal-seed"
      ...
      <GardenMenu
        ...
        mode={world}
        lifetimeCare={myGarden.lifetimeCare}
        giftPreview={getGuestPreviewImport(guestPreview)}
