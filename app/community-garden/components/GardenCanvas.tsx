...
function constrainRuntimeMovement(
  ...
  if (runtime.mode !== "personal") return clamped;
  const isOpen = (point: WorldPoint) =>
    isWithinRuntime(
      runtime,
      Math.floor(point.x / GARDEN_CONFIG.tileSize),
      Math.floor(point.y / GARDEN_CONFIG.tileSize),
    );
  if (isOpen(clamped)) return clamped;
  const slideX = { x: clamped.x, y: current.y };
  if (isOpen(slideX)) return slideX;
  const slideY = { x: current.x, y: clamped.y };
  if (isOpen(slideY)) return slideY;
  return current;
}
...
function getExpansionCandidateAt(
  runtime: Runtime,
  gridX: number,
  gridY: number,
) {
...
function isNextExpansionCell(runtime: Runtime, gridX: number, gridY: number) {
  return Boolean(
    getExpansionCandidateAt(runtime, gridX, gridY) &&
      !isPersonalBed(runtime, gridX, gridY),
  );
}
...
function applyPersonalGarden(runtime: Runtime, garden: MyGardenState) {
  ...
  if (runtime.mode === "personal") {
    const nextMary = getNearestPersonalGardenPosition(runtime, runtime.mary);
    if (nextMary.x !== runtime.mary.x || nextMary.y !== runtime.mary.y) {
      runtime.mary = { x: nextMary.x, y: nextMary.y };
      runtime.target = null;
      runtime.path = [{ ...runtime.mary }];
    }
  }
...
function canPlacePersonalElement(
  ...
  for (let y = gridY; y < gridY + candidate.height; y += 1) {
    for (let x = gridX; x < gridX + candidate.width; x += 1) {
      if (!isPersonalBed(runtime, x, y)) return false;
...
function getActionState(runtime: Runtime) {
  ...
  if (runtime.mode === "personal") {
    if (
      ...
      const expansion = getExpansionCandidateAt(
        runtime,
        runtime.selected.gridX,
        runtime.selected.gridY,
      );
      ...
      return {
        ...
        label: preview
          ? "Land expansion · Members"
          : care >= cost
            ? `Unlock · ${cost} Care`
            : `${cost - care} more Care to unlock`,
        enabled: !preview && !runtime.actionBusy && care >= cost,
      };
    }
...
function renderPersonalGardenShare(
  ...
  renderGarden(sourceContext, {
    ...
    personalGarden: {
      ...
      gardenHouseEnabled: false,
      gardenHouseUnreadCount: 0,
      unlockedParcels: garden.unlockedParcels,
      expansionCandidates: [],
      nextExpansion: null,
    },
  });
...
export const GardenCanvas = forwardRef<GardenCanvasHandle, GardenCanvasProps>(
  function GardenCanvas(
    ...
    useEffect(() => {
      onOpenGardenHouseRef.current = onOpenGardenHouse;
    }, [onOpenGardenHouse]);
    ...
    useEffect(() => {
      personalCommunityFlowersRef.current = personalCommunityFlowers;
    }, [personalCommunityFlowers]);

    useEffect(() => {
      showPersonalCommunityFlowersRef.current = showPersonalCommunityFlowers;
    }, [showPersonalCommunityFlowers]);
    ...
    const publishUi = useCallback(() => {
      ...
      const personalMapParcels =
        runtime.mode === "personal" && runtime.personalGarden
          ? (() => {
              const unlocked = runtime.personalGarden.unlockedParcels ?? [];
              const parcels = unlocked.length > 0
                ? unlocked
                : [{
                    parcelX: 0,
                    parcelY: 0,
              ...
              return parcels.map((parcel) => {
                const hasParcel = (parcelX: number, parcelY: number) =>
                  unlocked.some(
                    (candidate) =>
                      candidate.parcelX === parcelX &&
                      candidate.parcelY === parcelY,
                  );
                return {
                  key: `${parcel.parcelX}:${parcel.parcelY}`,
                  x: ((parcel.minX - mapBounds.minX) / mapWidth) * 100,
                  y: ((parcel.minY - mapBounds.minY) / mapHeight) * 100,
                  width: (parcel.width / mapWidth) * 100,
                  height: (parcel.height / mapHeight) * 100,
                  fenceTop:
                    unlocked.length === 0 ||
                    !hasParcel(parcel.parcelX, parcel.parcelY - 1),
                  fenceRight:
                    unlocked.length === 0 ||
                    !hasParcel(parcel.parcelX + 1, parcel.parcelY),
                  fenceBottom:
                    unlocked.length === 0 ||
                    !hasParcel(parcel.parcelX, parcel.parcelY + 1),
                  fenceLeft:
      ...
      const state: GardenUiState = {
        ...
        selectedGridX: runtime.selected?.gridX ?? null,
        selectedGridY: runtime.selected?.gridY ?? null,
        pathMapPoints,
        personalMapParcels,
        plantMapPoints,
        regionMapCells,
    ...
    useEffect(() => {
      ...
      if (mode === "house") {
        ...
        if (saved) {
          runtime.mary = getNearestPersonalGardenPosition(runtime, {
              ...
              const selectedExpansionCandidate =
                ...
                runtime.personalGarden.expansionCandidates?.find(
                  (candidate) =>
                    ...
                    selected.gridY >= candidate.minY &&
                    selected.gridY < candidate.minY + candidate.height,
                );
      ...
      const tick = (now: number) => {
        ...
        if (runtime.target) {
          ...
          if (distance <= Math.max(1, step)) {
            runtime.mary = constrainRuntimeMovement(
              runtime,
              runtime.mary,
              runtime.target,
            );
            ...
            const nextMary = constrainRuntimeMovement(runtime, runtime.mary, {
              x: runtime.mary.x + (dx / distance) * step,
              y: runtime.mary.y + (dy / distance) * step,
            });
            if (
              nextMary.x === runtime.mary.x &&
              nextMary.y === runtime.mary.y
            ) {
              runtime.target = null;
              runtime.statusMessage = "The fence marks the edge of your land.";
            } else {
              runtime.mary = nextMary;
              runtime.moving = true;
              runtime.hasMoved = true;
            }
          }
        ...
        renderGarden(ctx, {
          ...
          personalGarden: runtime.personalGarden
            ? {
                ...
                gardenHouseEnabled: true,
                gardenHouseUnreadCount: gardenHouseUnreadCountRef.current,
    ...
    function selectCell(gridX: number, gridY: number) {
      ...
      if (
        ...
        runtime.target = null;
        runtime.path = [];
        runtime.statusMessage = "Opening the Hall of Growth...";
        publishUi();
        onOpenGardenHouseRef.current();
        return;
      }
