...
function drawPixelShed(
  ...
  if (houseEnabled) {
    // A tiny trophy on the door marks the house as the Hall of Growth.
    ctx.fillStyle = "#e7b84e";
    ctx.fillRect(8, -23, 5, 5);
    ctx.fillRect(7, -22, 1, 3);
    ctx.fillRect(13, -22, 1, 3);
    ctx.fillRect(10, -18, 1, 3);
    ctx.fillRect(8, -15, 5, 1);
    ctx.fillStyle = "#fff0b3";
    ctx.fillRect(9, -22, 3, 1);
    if (houseUnreadCount > 0) {
...
function drawPersonalElement(
  ...
  } else if (element.elementType === "hedge") {
    ctx.fillStyle = "#3f5c3c";
    ctx.fillRect(-10, -12, 20, 14);
    ctx.fillStyle = "#58764c";
    ctx.fillRect(-9, -14, 18, 12);
    ctx.fillStyle = "#88a06b";
    ctx.fillRect(-7, -12, 6, 4);
    ctx.fillRect(2, -9, 5, 3);
    ctx.fillStyle = "#45623f";
    ctx.fillRect(-8, -4, 16, 4);
  } else if (element.elementType === "fern") {
    ctx.fillStyle = "#405d42";
    ctx.fillRect(-1, -17, 2, 19);
    ctx.fillStyle = "#4f714d";
    ctx.fillRect(-8, -13, 8, 3);
    ctx.fillRect(1, -15, 8, 3);
    ctx.fillRect(-11, -9, 11, 3);
    ctx.fillRect(1, -11, 12, 3);
    ctx.fillRect(-9, -5, 9, 3);
    ctx.fillRect(1, -7, 10, 3);
    ctx.fillStyle = "#7f9a68";
...
export function renderGarden(ctx: CanvasRenderingContext2D, state: RenderGardenState) {
  if (state.mode === "personal" && state.personalGarden) {
    ...
    drawPixelShed(
      ...
      state.viewport,
      state.zoom,
      Boolean(state.personalGarden.gardenHouseEnabled),
      state.personalGarden.gardenHouseUnreadCount ?? 0,
    );
