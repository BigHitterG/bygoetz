...
alter table public.basil_social_transfer_tokens
  ...
  check (purpose in ('upload', 'download', 'notify'));
...
declare
  secret text;
begin
  if p_purpose not in ('upload', 'download', 'notify') then
...
comment on constraint basil_social_transfer_purpose_check
  ...
  'One-time capabilities may upload a package, download an approved package, or send its private review email.';
