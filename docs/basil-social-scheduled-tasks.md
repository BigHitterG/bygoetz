...
> Preserve every reusable finished package before sending the review email. Upsert a row in `public.basil_social_evergreen_collections`, issue a collection-scoped token with `public.issue_basil_social_evergreen_transfer_token(collection_id, asset_count)`, and immediately run `pnpm social:archive-evergreen -- --collection-key=<key> --collection-id=<uuid> --transfer-token=<returned-token>`. Botanical lifecycle collections must include every locked-camera stage keyframe, the species profile, final MP4, poster, and production manifest; retain an alternate keyframe when it informed the final choice. Diagram collections retain the final PNG and production manifest. Game-mechanic collections retain the final MP4, poster, caption timing, and production manifest, but not their disposable intermediate capture frames. Verify every catalog row against its server-computed SHA-256 and confirm the `basil-social-evergreen` bucket remains private. Never print or store a transfer token. The durable collection definitions live in `content/basil-social/evergreen-archive.json`; append a new collection instead of overwriting an earlier one.
>
